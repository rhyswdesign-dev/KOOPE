-- Migration 026: Challenge rotation recovery
--
-- Re-applies the challenge refresh function for databases that already ran
-- migration 024, grants authenticated clients permission to call it, and runs it
-- immediately so expired challenge pools come back online.
...
WITH ranked_challenges AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY title, frequency
      ORDER BY expires_at DESC, updated_at DESC, created_at DESC, id DESC
    ) AS row_number
  FROM public.challenges
)
DELETE FROM public.challenges challenge
USING ranked_challenges ranked
...
DECLARE
  ...
  weekly_start   TIMESTAMPTZ := date_trunc('week', NOW() AT TIME ZONE 'UTC') AT TIME ZONE 'UTC';
...
BEGIN
  ...
  ('Recipe Archivist', 'Save 12 cocktail recipes this month', 'exploration', 'monthly', 'hard', 800, 'recipe_archivist', 'save_recipe', 12, 'albums-outline', '#00ACC1', monthly_expiry, ARRAY['recipe_id'])
  ON CONFLICT (title, frequency) DO UPDATE SET
    description = EXCLUDED.description,
    category = EXCLUDED.category,
    difficulty = EXCLUDED.difficulty,
...
GRANT EXECUTE ON FUNCTION public.refresh_active_challenges() TO authenticated;
