...
const TheVelvetCurtainScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [isStoryExpanded, setIsStoryExpanded] = useState(false);
  const { toggleSavedBar, isBarSaved } = useSavedItems();
  
  const bar = getBar('the_velvet_curtain');
  ...
  const barId = 'the-velvet-curtain';
  const isSaved = isBarSaved(barId);

  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out The Velvet Curtain! Wine-focused intimate atmosphere - A sophisticated wine experience.`,
        title: `The Velvet Curtain - Premium Bar Experience`,
      });
    } catch (error) {
        ...
        {bar.location && (
          <Section title="Location & Contact">
            <LocationMap
              ...
              height={220}
              onMarkerPress={(location) => log.info('TheVelvetCurtainScreen', 'Bar location pressed', { locationName: location.name })}
            />
          </Section>
        )}
...
export default TheVelvetCurtainScreen;