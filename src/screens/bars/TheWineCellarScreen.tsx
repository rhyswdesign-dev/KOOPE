...
const TheWineCellarScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [isStoryExpanded, setIsStoryExpanded] = useState(false);
  const { toggleSavedBar, isBarSaved } = useSavedItems();
  
  const bar = getBar('the_wine_cellar');
  ...
  const barId = 'the-wine-cellar';
  const isSaved = isBarSaved(barId);

  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out The Wine Cellar! Underground Elegance - Descend into Toronto's most elegant wine cellar with rare vintages.`,
        title: `The Wine Cellar - Premium Bar Experience`,
      });
    } catch (error) {
        ...
        {bar.location && (
          <Section title="Location & Contact">
            <LocationMap
              ...
              height={220}
              onMarkerPress={(location) => log.info('TheWineCellarScreen', 'Bar location pressed', { locationName: location.name })}
            />
          </Section>
        )}
...
export default TheWineCellarScreen;