...
const TheGildedLilyScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [isStoryExpanded, setIsStoryExpanded] = useState(false);
  const { toggleSavedBar, isBarSaved } = useSavedItems();
  
  const bar = getBar('the_gilded_lily');
  ...
  const barId = 'the-gilded-lily';
  const isSaved = isBarSaved(barId);

  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out The Gilded Lily! 1920s Glamour - A sophisticated jazz bar with live music and craft cocktails.`,
        title: `The Gilded Lily - Premium Bar Experience`,
      });
    } catch (error) {
        ...
        {bar.location && (
          <Section title="Location & Contact">
            <LocationMap
              ...
              height={220}
              onMarkerPress={(location) => log.info('TheGildedLilyScreen', 'Bar location pressed', { locationName: location.name })}
            />
          </Section>
        )}
...
export default TheGildedLilyScreen;