...
const SkylineLoungeScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [isStoryExpanded, setIsStoryExpanded] = useState(false);
  const { toggleSavedBar, isBarSaved } = useSavedItems();
  
  const bar = getBar('skyline_lounge');
  ...
  const barId = 'skyline-lounge';
  const isSaved = isBarSaved(barId);

  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out Skyline Lounge! Sky-High Luxury - Toronto's highest cocktail experience with breathtaking city views.`,
        title: `Skyline Lounge - Premium Bar Experience`,
      });
    } catch (error) {
        ...
        {bar.location && (
          <Section title="Location & Contact">
            <LocationMap
              ...
              height={220}
              onMarkerPress={(location) => log.info('SkylineLoungeScreen', 'Bar location pressed', { locationName: location.name })}
            />
          </Section>
        )}
...
export default SkylineLoungeScreen;