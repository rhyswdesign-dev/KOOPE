...
const TheTikiHutScreen: React.FC = () => {
  ...
  const barId = 'the-tiki-hut';
  const isSaved = isBarSaved(barId);

  const handleShare = async () => {
    try {
      await Share.share({
        message: `Check out The Tiki Hut! Island Paradise - Escape to the tropics with rum cocktails and tropical vibes.`,
        title: `The Tiki Hut - Premium Bar Experience`,
      });
    } catch (error) {
        ...
        {bar.location && (
          <Section title="Location & Contact">
            <LocationMap
              ...
              height={220}
              onMarkerPress={(location) => log.info('TheTikiHutScreen', 'Bar location pressed', { locationName: location.name })}
            />
          </Section>
        )}
...
export default TheTikiHutScreen;