...
function getRecommendationReason(item: CellarRecord): string {
  ...
  if (r === 'Hold')
    return "Current value is outpacing cost basis. This bottle retains collector upside — the secondary market hasn't fully priced in limited availability yet.";
  if (r === 'Open Soon')
    return 'The market premium is light on this expression. Better positioned as a considered pour than a long hold in the current cycle.';
...
function getVelocityDesc(v: 'Aggressive' | 'Moderate' | 'Stable'): string {
  if (v === 'Aggressive')
    return 'Recently acquired. The market for new expressions is active — watch comps over the next 60 days.';
  if (v === 'Moderate')
    return 'Settling into the collection. First secondary market cycle valuations typically emerge in this window.';
  ...
  return (
    <LinearGradient colors={['#1E130D', '#0F0A07']} style={styles.emptyWrap}>
      ...
      <Text style={styles.emptyBody}>
        Add bottles from your HomeBar to the Cellar, then they'll appear here with full collector
        intelligence.
      </Text>
    </LinearGradient>
  );
...
function CellarVaultTab() {
  ...
  const loadLots = useCallback(async () => {
    ...
    try {
      ...
      const sorted = Object.values(records).sort(
        (a, b) => getCollectorScore(b) - getCollectorScore(a),
      );
      setLots(sorted);
      setIdx(0);
    } catch {
  ...
  const handleRemove = () => {
    ...
    Alert.alert(
      'Remove from Vault',
      'This removes the collector record but keeps the bottle in Inventory.',
      [
      ...
      <SafeAreaView edges={['top']} style={styles.container}>
        <LinearGradient colors={['#24170F', '#130D09']} style={styles.lockedWrap}>
          ...
          <Text style={styles.lockedSub}>
            Individual lot intelligence — collector-grade detail on every bottle in your reserve.
          </Text>
        </LinearGradient>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.container}>
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={loading} onRefresh={loadLots} tintColor={colors.accent} />
        }
      >
        {/* ── lot navigator ─────────────────────────── */}
        {lots.length > 1 && (
          <View style={styles.lotNav}>
            ...
            >
              <Ionicons
                name="chevron-back"
                size={16}
                color={idx === 0 ? 'rgba(255,255,255,0.2)' : colors.accent}
              />
            </TouchableOpacity>
            <Text style={styles.lotNavLabel}>
              LOT {idx + 1} OF {lots.length}
            </Text>
            <TouchableOpacity
            ...
            >
              <Ionicons
                ...
                size={16}
                color={idx === lots.length - 1 ? 'rgba(255,255,255,0.2)' : colors.accent}
              />
            </TouchableOpacity>
          </View>
        )}
          ...
          <>
            ...
            <View style={styles.metricsCol}>
              ...
              <View style={styles.metricCard}>
                ...
                <Text style={styles.metricDesc}>
                  Score reflects data completeness and holding duration. Add tasting notes to
                  improve.
                </Text>
              </View>
              ...
              <View style={styles.metricCard}>
                ...
                <View style={styles.chipRow}>
                  <View style={styles.chip}>
                    <Text style={styles.chipText}>ORIGINAL BOX</Text>
                  </View>
                  <View style={styles.chip}>
                    <Text style={styles.chipText}>
                      FILL LEVEL {String(record.quantity || 'FULL').toUpperCase()}
                    </Text>
                  </View>
                </View>
              </View>
            ...
            <View style={styles.actionRow}>
              <TouchableOpacity
                ...
                onPress={() =>
                  Alert.alert('Coming soon', 'Market listing will be available in a future update.')
                }
              >
              ...
              <View style={styles.specsGrid}>
                {[
                  ...
                  [
                    'Notes',
                    record.cellarNotes
                      ? record.cellarNotes.slice(0, 40) +
                        (record.cellarNotes.length > 40 ? '…' : '')
                      : '—',
                  ],
                ].map(([label, value]) => (
              ...
              >
                ...
                <View style={styles.editChipRow}>
                  {(['full', 'half', 'low', 'empty'] as const).map((q) => (
                    ...
                    >
                      <Text
                        style={[styles.editChipText, editQty === q && styles.editChipTextActive]}
                      >
                ...
                <TextInput
                  ...
                  value={editTasting}
                  onChangeText={setEditTasting}
                  placeholder="Oak, dried fruit, peat…"
                  placeholderTextColor={colors.subtext}
                  multiline
                />
