...
**PHASE 0 EXIT:** RLS test script green · one payment system · one analytics SDK, consent-first · zero Firebase · kill list merged · one XP spine · two tiers · Made It live and reporting. _App Store submission-ready at every merge._
...
- **Actions row**: Add to Bar (Owned) · Want it (price capture inline — seeds the price journal) · **Gift mode** toggle ("scanning for someone?") → good-gift verdict + what _they_ could make (v1: taste-profile questionnaire-lite, 2 taps).
- **Shipped (2026-07-26).** The rebuild landed in place in `BottleDetailScreen.tsx` (~1,420 lines, down from 3,269) rather than as a new `AnswerCardScreen.tsx` — the existing screen is the Answer Card now. Two waves:
  - Already shipped ahead of this pass (a founder cut on 2026-07-18, per in-file comments): **value line** with range + sourced tooltip + full "you saw $32 ✓ good buy" verdict, capture at scan time (`ValueLine.tsx`, `valueVerdictService.ts`, `spottedPriceService.ts`, `capturePoint: 'at_scan'`) — the §B.5 open question is answered; **actions row** as three real peers including working gift mode, not a stub (`GiftModePanel.tsx`, `giftVerdictService.ts`); **identity block** merged near the top with detail behind "see more"; **dead Cellar Mode block** already deleted.
  - Shipped by this task: **the Hook**, extracted to `CocktailHookRail.tsx` and reordered up the screen, with the paywalled 4th recipe now a real item inside the `FlatList` row (a `{kind:'recipe'|'locked'}` union) instead of a bolted-on footer card; **scan-feedback/correction UI**, extracted to `ScanFeedbackPanel.tsx` and moved below the "see more" fold, with one judgment call: a slim amber "Not this bottle? Fix it" pill shows _above_ the fold, but only on low-confidence scans (`isLowConfidence`), so a shaky ID isn't hidden without a way out.
- **Accept (revised):** above-the-fold order and content match spec §B.1; gift mode and at-scan value verdict are real, not stubs. The `< 3s` render measurement and "every scan path terminates here" depend on 1.3 (below); the value-line range itself stays an estimate until licensed pricing data lands (1.2, Phase 4 territory per the Master Plan).
...
- Instrument the 4-layer pipeline (`barcodeService.ts`, `ocrService.ts`, `googleVisionService.ts`, manual): per-layer latency + success into a `scan_events` extension (see 1.5). Pre-warm camera on tab focus; degrade gracefully offline (`offlineService.ts`): identity from local cache, queue enrichment.
- **Shipped (2026-07-26)**, per [KOOPE-SCANNER-ANSWER-CARD-SPEC.md](KOOPE-SCANNER-ANSWER-CARD-SPEC.md) Part A:
  1. **Silent, always-on barcode decode** — the `barcodeOnly` route param, the barcode-mode UI (reticle, "Scanning for barcode automatically" hint, the barcode-found toast), and the "bottle not found → switch to barcode mode" fallback modal are all removed (`SmartScanScreen.tsx`, `CameraCapture.tsx`, `BottleNotFoundModal.tsx`, `CameraStack.tsx`; `ScanCounter.tsx` deleted as dead code once barcode mode went). The decoder now runs continuously with zero UI and fails silently — a miss produces no alert. `BottleDetailScreen.tsx`'s "wrong bottle" correction option changed from "Scan the barcode" to "Scan it again," matching the one-mode UX.
  2. **Race the barcode resolution** — `barcodeService.ts` now queries `bottle_barcode_mappings` and a new `spirits_cache` barcode-key lookup in parallel with the public grocery APIs, first-good-result-wins, with an `AbortController`-cancel on the rest. A real waterfall bug was fixed in the process: barcode-variant lookups (UPC-A's 2 variants) were serializing into consecutive 6s timeout windows; now fully parallel.
  3. **Two chained cloud calls already collapsed to one** before this task, via `supabase/functions/bottle-recognize` (commit `6e21067`) — option (b) from the spec, confirmed already shipped, not redone.
  4. **1024px downscale already shipped** before this task (`googleVisionService.ts`, commit `d6ea7bf`) — confirmed, not redone.
  5. **Pre-network bad-photo heuristics** — `imageQualityService.ts` went from 2 checks to 4 (`too_dark`, `too_bright` [new], `too_blurry`, `too_far` [new, via an edge-density heuristic]), each with specific retry copy, from one image pass (no added latency).
  - Two related bugs fixed as part of making barcode silent: telemetry was firing failed-scan events on every unstable decode attempt (poisoning the success-rate KPI this task is supposed to produce) — now only a winning silent scan fires telemetry; and a late-arriving barcode result could yank the user off a photo-path result they'd already captured — fixed with a scan-run id that invalidates stale in-flight lookups.
  - Judgment call: tier-1 sources (`bottle_barcode_mappings` + `spirits_cache`) race each other via `Promise.all` rather than literal first-responder, so a heuristic public-API-sourced cache row can't outrank a user-corrected mapping — both still race against and beat the public APIs, which is where the real latency was.
- Dashboard query: scan success rate and p50/p95 time-to-answer become standing metrics — not yet wired to a Mixpanel dashboard (telemetry now emits correctly; the dashboard itself is a follow-up).
- **Accept:** p50 < 3s in the common (vision, cache-warm) path per the spec's §A.4 latency budget — not yet measured on real devices; success > 90% now (95% is the standing company KPI) — not yet measured; `npx tsc --noEmit`, lint, and the 244-test suite pass clean on the rebuild.
...
- **Accept:** the query "brand X: scans, store-context %, want-conversion %, top alternative scanned in same session" runs today — that query _is_ the future brand product.
...
### 1.7 Notification rails — Phase A (2026-07-26, folded in from [KOOPE-NOTIFICATION-PLAYBOOK.md](KOOPE-NOTIFICATION-PLAYBOOK.md)) — **L**

Zero-infra, local-first — unblocked now (Hosting Planner and What Can I Make already ship). Per the Playbook's own Phase A sequencing:

1. **Wire deep links** — `notificationService.ts`'s `actionUrl` is currently logged, not navigated (`notificationService.ts:260`); connect to the nav service.
2. **`NotificationPlanner` module** — the frequency governor (≤4 non-transactional/week, ≤1/day, none on a day the user already opened the app). Individual features stop calling `scheduleNotificationAsync` directly; recomputes the next 7 days on every app open.
3. **Hosting countdown sequence** (T-72h/T-24h/day-of/post-event) — the flagship send, ships before any L2 habit send (Playbook standing decision #2); scheduled at event creation in the Hosting Planner.
4. **Friday maker prompt** (Fri 4:30pm, personalized from shelf + recipe matching, only if no make logged this week) — the single most important habit send per the Playbook; directly manufactures the Weekly Makers North Star.
5. **Measurement** — mirror Scheduled/Received/Tapped to Mixpanel with `{type, layer, slot}`; kill rule: any recurring send under 2% tap rate for two consecutive months gets cut.
6. **Copy pass** — rewrite the Duolingo-style guilt copy ("Don't break your streak!") per the Playbook §4 voice covenant; remap preference categories to My events / My progress / Weekly drops / Occasions & featured (last one defaults off).

- **Accept:** every notification deep-links to a real screen; a user cannot receive more than 4 non-transactional sends in a week (CI-tested in the planner, per Playbook scorecard); Friday prompt and hosting countdown are live and logging to Mixpanel.
- **Shipped (2026-07-26).** `navigationRef.ts` + a `NOTIFICATION_ROUTES` table in `deepLinking.ts` (with a pending-navigation queue for cold-start taps and `homegameadvantage://`→`koope://` scheme migration) close item 1. `notificationBudget.ts` (pure, CI-testable) + `notificationPlanner.ts` (candidate generation: Friday prompt, weekend host seed, streak protector, weekly drop, onboarding D1/D3/D7, winback) + `NotificationPlannerRunner.tsx` (runs on mount + app-foreground) close item 2 — every scheduler in `notificationService.ts` now routes through one `scheduleSend()`; the old ungoverned `scheduleStreakReminder()` is deleted. `scheduleHostingCountdown()` (T-72h/T-24h/day-of/post-event, idempotent per event, wired to `HostingScreen`'s new "when is it?" date picker) closes item 3. The Friday prompt (item 4) personalizes from a local shelf table, e.g. _"You've got gin, campari, and sweet vermouth — that's a Negroni, and you know it."_ Items 5 (`Notification Scheduled/Received/Tapped` with `{type, layer, slot}`) and 6 (streak-guilt copy rewritten; preferences remapped to My events/My progress/Weekly drops/Occasions & featured, last one defaulting off) are both done. Server push (Phase B) was correctly left untouched.
  - Two judgment calls worth knowing: hosting events previously had no date field at all (added `eventDate?: string` + a lightweight Alert-based chooser rather than pulling in a date-picker dependency — a real picker is an obvious follow-up); and the permission-ask flow, which was firing on launch (a Playbook violation), was tightened to ask only at two value moments (first non-empty What Can I Make result, hosting-plan save) with a primer first.
  - 25 new tests (`notificationBudget.test.ts`, `notificationPlanner.test.ts`) assert the ≤4/week governor and that no candidate carries banned copy or a non-`koope://` link.
...
### 2.1 RevenueCat activation & hardening — **L** _(honors the standing "when payments go live" gate — this is that moment)_
...
- **Accept:** end-to-end sandbox purchase/restore on iOS build; tier flips everywhere within one app session; founders counter decrements server-side. _(Per standing decision, live testing timing stays deferred until payments actually go live — build it ready.)_
...
1.4 Tonight, Hosting Planner (shipped) ─→ 1.7 notification rails (unblocked now)
```
